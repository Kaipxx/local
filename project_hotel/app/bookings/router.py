...
@router.get("/get")
async def get_booking(user: Users = Depends(get_current_user)) -> list[SBooking]:  # принимаем юзера

    return await BookingDAO.find_all(user_id=user.id)


@router.get("/del")
async def del_bookings(user: Users = Depends(get_current_user)) -> list[SBooking]:  # принимаем юзера
